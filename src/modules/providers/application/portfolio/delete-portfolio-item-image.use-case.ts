...
@Injectable()
export class DeletePortfolioItemImageUseCase {
  constructor(
  ...
  async execute(
    ...
    if (!image) throw new NotFoundException('Image not found on this item');

    await this.portfolioItemRepository.deleteImage(imageId);
    await this.imageUploader.delete(image.publicId).catch(() => undefined);
  }
}
